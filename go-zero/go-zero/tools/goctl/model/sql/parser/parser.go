...
func convertColumns(columns []*parser.Column, primaryColumn string, strict bool) (Primary, map[string]*Field, error) {
	...
	for _, column := range columns {
		...
		fmt.Println(conv.String(column))
...
func ConvertDataType(table *model.Table, strict bool) (*Table, error) {
	...
	primaryDataType, thirdPkg, containsPQ, err := converter.ConvertStringDataType(table.PrimaryKey.DataType,
		table.PrimaryKey.ColumnType, isPrimaryDefaultNull, isPrimaryUnsigned, strict)
	if err != nil {
		return nil, err
	}
...
func getTableFields(table *model.Table, strict bool) (map[string]*Field, error) {
	...
	for _, each := range table.Columns {
		...
		dt, thirdPkg, containsPQ, err := converter.ConvertStringDataType(each.DataType, each.ColumnType, isDefaultNull, isPrimaryUnsigned, strict)
		if err != nil {
			return nil, err
		}
		columnSeqInIndex := 0
