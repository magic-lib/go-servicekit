...
func ConvertStringDataType(dataBaseType string, databaseColumnType string, isDefaultNull, unsigned, strict bool) (
	goType string, thirdPkg string, isPQArray bool, err error) {
	if env.UseExperimental() {
		customTp, thirdImport := convertDatatypeWithConfig(dataBaseType, databaseColumnType, isDefaultNull, unsigned)
		//fmt.Println(strings.ToLower(dataBaseType), isDefaultNull, " convertDatatypeWithConfig:", customTp)
		if len(customTp) != 0 {
			return customTp, thirdImport, false, nil
		}

		tp, ok := commonMysqlDataTypeMapString[strings.ToLower(dataBaseType)]
		//fmt.Println(strings.ToLower(dataBaseType), " useExperimental:", tp)
		if !ok {
	...
	// the following are the old version compatibility code.
	tp, ok := commonMysqlDataTypeMapString[strings.ToLower(dataBaseType)]
	//fmt.Println(strings.ToLower(dataBaseType), ":", tp)
	if !ok {
		return "", "", false, fmt.Errorf("unsupported database type: %s", dataBaseType)
	}
...
func convertDatatypeWithConfig(dataBaseType string, databaseColumnType string, isDefaultNull, unsigned bool) (string, string) {
	externalConfig, err := config.GetExternalConfig()
	if err != nil {
		return "", ""
	}
	var opt config.ModelTypeMapOption
	var ok bool
	if databaseColumnType != "" {
		opt, ok = externalConfig.Model.TypesMap[strings.ToLower(databaseColumnType)]
	}
	if !ok {
