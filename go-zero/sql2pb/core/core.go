...
func (s *Schema) String() string {
	...
	buf.WriteString("\n")
	lo.ForEach(s.Imports, func(item string, index int) {
	...
	for _, m := range s.Messages {
		funcTpl += "\t //-----------------------" + m.Comment + "----------------------- \n"
		funcTpl += "\t rpc Insert" + m.Name + "(Insert" + m.Name + "Req) returns (Insert" + m.Name + "Resp); \n"
		...
		funcTpl += "\t rpc Find" + m.Name + "ById(Find" + m.Name + "ByIdReq) returns (Find" + m.Name + "ByIdResp); \n"
		funcTpl += "\t rpc List" + m.Name + "(List" + m.Name + "Req) returns (List" + m.Name + "Resp); \n"
	}
	funcTpl = funcTpl + "\n}"
...
func (m Message) GenRpcAddReqRespMessage(buf *bytes.Buffer) {
	...
	name := stringx.From(mOrginName).ToCamelWithStartLower()
	if m.Style == fieldStyleToSnake {
		name = stringx.From(mOrginName).ToSnake()
	}
	m.Fields = []MessageField{
		{Typ: mOrginName, Name: name, tag: 1, Comment: name},
	}
...
func (m Message) GenRpcGetByIdReqMessage(buf *bytes.Buffer) {
	...
	m.Name = "Find" + mOrginName + "ByIdReq"
	m.Fields = []MessageField{
		{Name: "id", Typ: "int64", tag: 1, Comment: "id"},
	}
	buf.WriteString(fmt.Sprintf("%s\n", m))
	...
	m.Name = "Find" + mOrginName + "ByIdResp"
